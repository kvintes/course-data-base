...
create schema if not exists it_2;
SET search_path TO it_2,public;
SHOW search_path;
--      1задание      --
...
--SET search_path TO it_2, public; -- проишем путь к схеме
-- 1шаг создаем таблицу Альпинист-Восхождение +
create table if not EXISTS it_2.Альпинист_Восхождение
(
...
CREATE SEQUENCE IF NOT EXISTS it_2.Альпинист_Восхождение_Альп_seq MINVALUE 0;
...
CREATE SEQUENCE IF NOT EXISTS it_2.Альпинист_Восхождение_Восх_seq MINVALUE 1000;
...
create table if not EXISTS it_2.Альпинисты
(
    ID_Альпиниста integer not null
    , ФИО text not null
...
CREATE SEQUENCE IF NOT EXISTS it_2.Альпинисты_seq MINVALUE 0;
...
create table if not EXISTS it_2.Восхождения
(
    ID_Восхождения integer not null
...
CREATE SEQUENCE IF NOT EXISTS it_2.Восхождения_seq MINVALUE 0;
...
--drop table if exists Вершины;
create table if not EXISTS it_2.Вершины
(
    ID_Вершины integer not null
...
-- 13шаг Вершины возможность вставки кортежа без указания первичного ключа +
CREATE SEQUENCE IF NOT EXISTS it_2.Вершины_seq MINVALUE 0;
...
INSERT INTO Вершины (Название, Высота, Страна, Регион) values ('БрусЭль', 5642, 'Россия', 'Кабардино-Балкария');
...
INSERT INTO Восхождения (Дата_начала, Дата_завершения, ID_Вершины) values ('2021-09-30', '2021-11-28', 3);
...
INSERT INTO Альпинист_Восхождение (ID_Альпиниста, id_Восхождения) values (3, 16);
...
INSERT INTO Вершины (id_Вершины, Название, Высота, Страна, region_id) values (2444, 'Балии', 4007, 'Россия', 1);
INSERT INTO Вершины (id_Вершины, Название, Высота, Страна, region_id) values (2446, 'Гидаач', 4020, 'Россия', 2);
...
(
    select 
        Альпинисты.ID_Альпиниста, Альпинисты.ФИО, Альпинисты.Телефон
        , Восхождения.Дата_начала
        , ROUND(date_part('day', current_timestamp - Восхождения.Дата_начала)) as days_per_start
        , Вершины.Название, Вершины.Высота, Вершины.Страна
    from Альпинисты
    inner join Альпинист_Восхождение on Альпинист_Восхождение.id_Альпиниста = Альпинисты.ID_Альпиниста
    inner join Восхождения on Восхождения.id_Восхождения = Альпинист_Восхождение.id_Восхождения
    inner join Вершины on Вершины.id_Вершины = Восхождения.id_Вершины
