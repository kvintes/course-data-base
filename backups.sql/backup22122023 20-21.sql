...
select 
	pd_products.id
	, pd_products.price
	, f_new_price_by_disconts(pd_products.id, 20) as new_price
from pd_products
...
select 
	pd_employees.id
	, f_count_exec_paid_orders_By_emp(pd_employees.id, 7)
	, f_count_exec_paid_orders_managedBy_emp(pd_employees.id, 7)
from pd_employees
;
...
END;
$$ LANGUAGE plpgsql;
select f_count_orders_per_month(7), 7 as month;
...
select distinct
    ...
    , f_costs_product_inOrder(pd_products.id, '2023-09-30'::date) as max_costs_September
from pd_products
;
...
$$;
DELETE FROM pd_orders * where pd_orders.id = 60100;
CALL insert_data_pd_orders_copy(6010);
select * from pd_orders
where id = 6010 or id = 60100;
DELETE FROM pd_orders * where pd_orders.id = 60110;
CALL insert_data_pd_orders_copy(6011);
select * from pd_orders
where id = 6011 or id = 60110;
DELETE FROM pd_orders * where pd_orders.id = 60120;
